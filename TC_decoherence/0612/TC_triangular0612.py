...
for ids in my_tasks:
    ...
    for ip, p in enumerate(p_list):
        while ptr < L and u_sorted[ptr] < p:
            ...
            NA = negativity_E_fast(active_MR, nsdd, Lv, A_idx)
            NB = negativity_E_fast(active_MR, nsdd, Lv, B_idx)
            NC = negativity_E_fast(active_MR, nsdd, Lv, C_idx)
            NAB = negativity_E_fast(active_MR, nsdd, Lv, AB_idx)
            NBC = negativity_E_fast(active_MR, nsdd, Lv, BC_idx)
            NCA = negativity_E_fast(active_MR, nsdd, Lv, CA_idx)
            NABC = negativity_E_fast(active_MR, nsdd, Lv, ABC_idx)
            ...
            #     anti = anticommutes(row, logical_Zy)
        ...
        logical_Xx_val = anticommutes(active_MR[nsdd-2], logical_Zy)
        anti_last1 = anticommutes(active_MR[nsdd-1], logical_Zy)

        # print(
        #     f"logical_Xx_val={logical_Xx_val}"
        # )
        ...
        # print("NA=", NA, "NB=", NB, "NC=", NC, "NAB=", NAB, "NBC=", NBC, "NCA=", NCA, "NABC=", NABC,
        #       "TEN=", TEN, "logical_Xx_val=", logical_Xx_val,flush=True)
        # print(
        #     f"p={p:.3f}, TEN={TEN:.1f}, logical_Xx_val={logical_Xx_val:.1f}",
        #     flush=True
        # )
        ...
        logical_Xx_val_list.append(logical_Xx_val)
        ...
        if len(TEN_list) > 1:
            ...
            logical_Xx_val_aved = np.mean(logical_Xx_val_list)
            logical_Xx_val_vard = np.var(logical_Xx_val_list)
            logical_Xx_val_errord = np.std(logical_Xx_val_list, ddof=1) / np.sqrt(len(logical_Xx_val_list))
            ...
            logical_Xx_val_aved = logical_Xx_val_list[0]
            logical_Xx_val_vard = 0.0
            logical_Xx_val_errord = 0.0
            ...
            logical_Xx_val_aved = np.nan
            logical_Xx_val_vard = np.nan
            logical_Xx_val_errord = np.nan
        ...
        local_results[key].append(
            (TEN, R2x, R2z, R2x_loop, R2z_loop, dep_x, dep_z, logical_Xx_val)
        )
...
if rank == 0:
    ...
    for p in sorted(merged_results):
        ...
        data_set["logical_Xx_val_ave"].append(logical_Xx_val_ave)
        data_set["logical_Xx_val_err"].append(logical_Xx_val_err)
        data_set["logical_Xx_val_var"].append(logical_Xx_val_var)
    ...
    logical_Xx_val_ave=np.array(data_set["logical_Xx_val_ave"]),
    logical_Xx_val_err=np.array(data_set["logical_Xx_val_err"]),
    logical_Xx_val_var=np.array(data_set["logical_Xx_val_var"])
    )
