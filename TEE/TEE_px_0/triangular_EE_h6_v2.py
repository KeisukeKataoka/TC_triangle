...
for (tmp_i, ids) in my_tasks:
    ...
    #NT=10*L #4*Lでなくて6*L?? total time step
    NT=3*L
    ...
    # stabilizer matrixを外部ファイルから読み込む
    MR0_path = f"MR_Lx_{Lx}_pg_{0.75:.2f}.npy"  # 固定で pg=0.75 のファイル
    MR0 = np.load(MR0_path).astype(np.uint32)
    assert MR0.shape == (L, 2*L), f"shape mismatch: {MR0.shape} != {(L, 2*L)}"
    ...
    for ip in range(Np):  # pg を順に変えながらMR引き継ぎ    
        ...
        for it in range(NT):
            ...
            if it==NT-1:
                ...
                for Ax in range(Lx):
                    for Ay in range(Ly):
                        ...
                        B3 = (subset6(L, Bx, By, Lx, Ly, p_indd) |
                            ...
                            subset6(L, Bx % Lx , (By - 3) % Ly, Lx, Ly, p_indd)) - A3
                        C3 = (subset6(L, Cx, Cy, Lx, Ly, p_indd) |
                              subset6(L, (Cx + 2) % Lx, (Cy + 1) % Ly, Lx, Ly, p_indd)|
                              subset6(L, (Cx + 3) % Lx, (Cy + 3) % Ly, Lx, Ly, p_indd)) - A3 - B3
                        ...
                        B4 = (subset6(L, Bx, By, Lx, Ly, p_indd) |
                            subset6(L, (Bx - 1) % Lx, (By - 2) % Ly, Lx, Ly, p_indd) |
                            subset6(L, Bx % Lx , (By - 3) % Ly, Lx, Ly, p_indd)|
                        ...
                        # h5
                        A5 = (subset6(L, Ax, Ay, Lx, Ly, p_indd) |
                            subset6(L, (Ax - 1) % Lx, (Ay + 1) % Ly, Lx, Ly, p_indd) |
                            subset6(L, (Ax - 3) % Lx, Ay, Lx, Ly, p_indd) |
                            subset6(L, (Ax - 4) % Lx, (Ay - 2) % Ly, Lx, Ly, p_indd)|
                            subset6(L, (Ax - 5) % Lx, (Ay - 4) % Ly, Lx, Ly, p_indd))
                        B5 = (subset6(L, Bx, By, Lx, Ly, p_indd) |
                            subset6(L, (Bx - 1) % Lx, (By - 2) % Ly, Lx, Ly, p_indd) |
                            subset6(L, Bx % Lx , (By - 3) % Ly, Lx, Ly, p_indd)|
                            subset6(L, (Bx + 2) % Lx, (By - 2) % Ly, Lx, Ly, p_indd)|
                            subset6(L, (Bx + 4) % Lx, (By - 1) % Ly, Lx, Ly, p_indd)) - A5
                        C5 = (subset6(L, Cx, Cy, Lx, Ly, p_indd) |
                              subset6(L, (Cx + 2) % Lx, (Cy + 1) % Ly, Lx, Ly, p_indd)|
                              subset6(L, (Cx + 3) % Lx, (Cy + 3) % Ly, Lx, Ly, p_indd)|
                              subset6(L, (Cx + 2) % Lx, (Cy + 4) % Ly, Lx, Ly, p_indd)|
                              subset6(L, (Cx + 1) % Lx, (Cy + 5) % Ly, Lx, Ly, p_indd)) - A5 - B5
                        ...
                        SB5 = EE_cal(MR, B5, L)
                        SC5 = EE_cal(MR, C5, L)
                        SAB5 = EE_cal(MR, AB5, L)
                        SBC5 = EE_cal(MR, BC5, L)
                        SCA5 = EE_cal(MR, CA5, L)
                        SABC5 = EE_cal(MR, ABC5, L)
                
                        TEE5 = SA5 + SB5 + SC5 - SAB5 - SBC5 - SCA5 + SABC5

                        # h6
                        A6 = (subset6(L, Ax, Ay, Lx, Ly, p_indd) |
                            subset6(L, (Ax - 1) % Lx, (Ay + 1) % Ly, Lx, Ly, p_indd) |
                            subset6(L, (Ax - 3) % Lx, Ay, Lx, Ly, p_indd) |
                            subset6(L, (Ax - 4) % Lx, (Ay - 2) % Ly, Lx, Ly, p_indd)|
                            subset6(L, (Ax - 5) % Lx, (Ay - 4) % Ly, Lx, Ly, p_indd)|
                            subset6(L, (Ax - 4) % Lx, (Ay - 5) % Ly, Lx, Ly, p_indd))
                        B6 = (subset6(L, Bx, By, Lx, Ly, p_indd) |
                            subset6(L, (Bx - 1) % Lx, (By - 2) % Ly, Lx, Ly, p_indd) |
                            subset6(L, Bx % Lx , (By - 3) % Ly, Lx, Ly, p_indd)|
                            subset6(L, (Bx + 2) % Lx, (By - 2) % Ly, Lx, Ly, p_indd)|
                            subset6(L, (Bx + 4) % Lx, (By - 1) % Ly, Lx, Ly, p_indd)|
                            subset6(L, (Bx + 5) % Lx, (By + 1) % Ly, Lx, Ly, p_indd)) - A6
                        C6 = (subset6(L, Cx, Cy, Lx, Ly, p_indd) |
                              subset6(L, (Cx + 2) % Lx, (Cy + 1) % Ly, Lx, Ly, p_indd)|
                              subset6(L, (Cx + 3) % Lx, (Cy + 3) % Ly, Lx, Ly, p_indd)|
                              subset6(L, (Cx + 2) % Lx, (Cy + 4) % Ly, Lx, Ly, p_indd)|
                              subset6(L, (Cx + 1) % Lx, (Cy + 5) % Ly, Lx, Ly, p_indd)|
                              subset6(L, (Cx - 1) % Lx, (Cy + 4) % Ly, Lx, Ly, p_indd)) - A6 - B6
                        ...
                        SB6 = EE_cal(MR, B6, L)
                        SC6 = EE_cal(MR, C6, L)
                        SAB6 = EE_cal(MR, AB6, L)
                        SBC6 = EE_cal(MR, BC6, L)
                        SCA6 = EE_cal(MR, CA6, L)
                        SABC6 = EE_cal(MR, ABC6, L)
                
                        TEE6 = SA6 + SB6 + SC6 - SAB6 - SBC6 - SCA6 + SABC6
                ...
                print(f"rank={rank:>3} Lx=Ly={Lx:>2} pg={pg:>6.3f} ids={ids:>4}", 
                      f"TEE1={TEE1:>6.3f} TEE2={TEE2:>6.3f} TEE3={TEE3:>6.3f}  TEE4={TEE4:>6.3f}  TEE5={TEE5:>6.3f}  TEE6={TEE6:>6.3f}", flush=True)
                TEE1_list=np.append(TEE1_list,TEE1)
                TEE2_list=np.append(TEE2_list,TEE2)
                TEE3_list=np.append(TEE3_list,TEE3)
                TEE4_list=np.append(TEE4_list,TEE4)
                TEE5_list=np.append(TEE5_list,TEE5)
                TEE6_list=np.append(TEE6_list,TEE6)
        ...
        if key not in local_results:
            local_results[key] = []
        local_results[key].append((TEE1, TEE2, TEE3, TEE4, TEE5, TEE6)) 
...
if rank == 0:
    ...
    for key in merged_results:
        ...
        ave5, err5, var5 = ave_err_var(TEE5_list)
        ave6, err6, var6 = ave_err_var(TEE6_list)
        ...
        print(f"  TEE4: ave={ave4:.4f} ± {err4:.4f}, var={var4:.4f}")
        print(f"  TEE5: ave={ave5:.4f} ± {err5:.4f}, var={var5:.4f}")
        print(f"  TEE6: ave={ave6:.4f} ± {err6:.4f}, var={var6:.4f}")
    ...
    for Lx in data_set:
        ...
        np.savez_compressed(f"Lx_{Lx}_Nd_{Nd}_NT_{NT}_px_{px}_MPI_hexagonal6.npz", 
                            pg=data_set[Lx]["pg"],
                            ...
                            TEE5_ave=data_set[Lx]["TEE5_ave"],
                            TEE5_err=data_set[Lx]["TEE5_err"],
                            TEE5_var=data_set[Lx]["TEE5_var"],
                            TEE6_ave=data_set[Lx]["TEE6_ave"],
                            TEE6_err=data_set[Lx]["TEE6_err"],
                            TEE6_var=data_set[Lx]["TEE6_var"])
