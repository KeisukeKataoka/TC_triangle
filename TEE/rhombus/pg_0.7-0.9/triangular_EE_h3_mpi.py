...
for (tmp_i, ip, ids) in my_tasks:
    ...
    NT=20*L #4*Lでなくて6*L?? total time step
    ...
    for it in range(NT):
        ...
        if it==NT-1:
            ...
            #
            A = subset2a(L,Ax,Ay,Lx,Ly,p_indd)
            B = subset2b(L,Bx,By,Lx,Ly,p_indd) - A
            C = subset2c(L,Cx,Cy,Lx,Ly,p_indd) - B
...
if rank == 0:
    ...
    for Lx in data_set:
        np.savez_compressed(f"px_{px}_Lx_{Lx}_NT_{NT}_Nd_{Nd}_MPI_results.npz", 
                            pg=data_set[Lx]["pg"],
