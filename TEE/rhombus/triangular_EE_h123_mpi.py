...
for (tmp_i, ip, ids) in my_tasks:
    ...
    for it in range(NT):
        ...
        if it==NT-1:
            ...
            print(f"rank={rank:>2} Lx=Ly={Lx:>2} pg={pg:>6.3f} ids={ids:>4} TEE1={TEE1:>2} TEE2={TEE2:>2} TEE3={TEE3:>2}", flush=True)
    ...
    if key not in local_results:
        local_results[key] = []
    local_results[key].append((TEE1, TEE2, TEE3)) 
...
if rank == 0:
    ...
    # npzファイル保存
    for Lx in data_set:
        np.savez_compressed(f"px_{px}_Lx_{Lx}_Nd_{Nd}_NT_{NT}_MPI_results.npz", 
                            pg=data_set[Lx]["pg"],
