...
def subset2a(Ld,Ax,Ay,Lx,Ly,p_indd):
    i = idx(Ax,Ay,Lx,Ly)
    Asub_set = set()
    for j in range(3):
...
def subset2b(Ld,Ax,Ay,Lx,Ly,p_indd):
    i = idx(Ax,Ay,Lx,Ly)
    Bsub_set = set()
    for j in range(3):
        Bsub_set.add(p_indd[i][j]) #三角形
        Bsub_set.add(p_indd[i - Lx + Lx*Ly][j]) #逆三角形
        #print(p_indd[i][j])
        #print(p_indd[i - Lx + Lx*Ly][j])
    return Bsub_set

def subset2c(Ld,Ax,Ay,Lx,Ly,p_indd):
    i = idx(Ax,Ay,Lx,Ly)
    Csub_set = set()
    for j in range(3):
        Csub_set.add(p_indd[i][j]) #三角形
        Csub_set.add(p_indd[i +1 + Lx*Ly][j]) #逆三角形
        #print(p_indd[i][j])
        #print(p_indd[i - Lx + Lx*Ly][j])
    return Csub_set

def subset4a(Ld,Ax,Ay,Lx,Ly,p_indd,subset2a):
    Asub_set = set()
    Asub_set = subset2a(L,Ax,Ay,Lx,Ly,p_indd)|subset2a(L,Ax+1,Ay,Lx,Ly,p_indd)|subset2a(L,Ax,Ay+1,Lx,Ly,p_indd)|subset2a(L,Ax+1,Ay+1,Lx,Ly,p_indd)
    #print(Asub_set)
    return Asub_set

def subset4b(Ld,Bx,By,Lx,Ly,p_indd,subset2b):
    Bsub_set = set()
    Bsub_set = subset2b(L,Bx,By,Lx,Ly,p_indd)|subset2b(L,Bx,By+1,Lx,Ly,p_indd)|subset2b(L,Bx-1,By,Lx,Ly,p_indd)|subset2b(L,Bx-1,By-1,Lx,Ly,p_indd)
    #print(Bsub_set)
    return Bsub_set

def subset4c(Ld,Cx,Cy,Lx,Ly,p_indd,subset2c):
    Csub_set = set()
    Csub_set = subset2c(L,Cx,Cy,Lx,Ly,p_indd)|subset2c(L,Cx+1,Cy,Lx,Ly,p_indd)|subset2c(L,Cx,Cy-1,Lx,Ly,p_indd)|subset2c(L,Cx-1,Cy-1,Lx,Ly,p_indd)
    #print(Csub_set)
    return Csub_set
...
for (tmp_i, ip, ids) in my_tasks:
    ...
    alpha=4
    NT=alpha*L #4*Lでなくて6*L?? total time step
    ...
    for it in range(NT):
        ...
        if it==NT-1:
            ...
            #正六角形をひし形分割
            A1 = subset2a(L,Ax,Ay,Lx,Ly,p_indd)
            B1 = subset2b(L,Bx,By,Lx,Ly,p_indd) - A1
            C1 = subset2c(L,Cx,Cy,Lx,Ly,p_indd) - A1 - B1
            ...
            A2 = subset4a(L,Ax,Ay,Lx,Ly,p_indd,subset2a)
            B2 = subset4b(L,Bx,By,Lx,Ly,p_indd,subset2b) - A2
            C2 = subset4c(L,Cx,Cy,Lx,Ly,p_indd,subset2c) - A2 - B2
            ...
            print(f"rank={rank:>2} Lx=Ly={Lx:>2} pg={pg:>6.3f} ids={ids:>4} TEE1={TEE1:>2} TEE2={TEE2:>2}", flush=True)
    ...
    if key not in local_results:
        local_results[key] = []
    local_results[key].append((TEE1, TEE2)) 
...
if rank == 0:
    ...
    # npzファイル保存
    for Lx in data_set:
        np.savez_compressed(f"Lx_{Lx}_Nd_{Nd}_NT_{alpha}T_px_{px}_MPI_rhombus.npz", 
                            pg=data_set[Lx]["pg"],
