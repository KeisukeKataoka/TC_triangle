...
for (tmp_i, ip, ids) in my_tasks:
    ...
    px=(pl-pg-ps)*ip/(Np-1) + ps
    pz=1-px-pg

    # --- 数値安定化: クリップ＆正規化 ---
    probs = np.array([pg, px, pz], dtype=float)

    # 負の微小値を0に丸める（必要なら小さめのepsに変更）
    eps = 1e-15
    probs[probs < 0] = 0.0

    # 合計を1に正規化（丸めで1±εになっているのを補正）
    s = probs.sum()
    if s == 0.0:
        # 全て0になることは本来ないが、保険として何か一つに重みを持たせる
        probs = np.array([1.0, 0.0, 0.0])
    else:
        probs /= s

    # 以降は probs を使う
    #mtype = np.random.choice(todo, p=probs)
    
    p_list=np.append(p_list,px)
    # stabilizer matrix
    ...
    todo=[1,2,3]
    for it in range(NT):
        mtype= np.random.choice(todo,size=None,replace=True,p=probs)
        ...
        if it==NT-1:
            ...
            print(f"rank={rank:>3} Lx=Ly={Lx:>2} px={px:>6.3f} ids={ids:>4} TEE={TEE:>2} R2x={R2x:>6.3f} R2z={R2z:>6.3f} R2x_loop={R2x_loop:>6.3f} R2z_loop={R2z_loop:>6.3f}  dep_x={dep_x:>6.3f} dep_z={dep_z:>6.3f}", flush=True)
    ...
    key1="Lx="+"{:}".format(Lx)+"_Ly="+"{:}".format(Ly)+"_p="+"{:.2f}".format(px)

    key = (Lx, round(px, 3))
    if key not in local_results:
        local_results[key] = []
    local_results[key].append( (TEE, R2x, R2z, R2x_loop, R2z_loop, dep_x, dep_z) )
...
if rank == 0:
    ...
    for key in merged_results:
        ...
        print(f"Lx={Lx}, px={px:.3f} : TEE={TEE_ave:.4f}, x-string={R2x_ave:.4f}, z-string={R2z_ave:.4f}, x-loop={R2x_loop_ave:.4f}, z-loop={R2z_loop_ave:.4f}, dep_x={dep_x_ave:.4f}, dep_z={dep_z:.4f}")
        
        # 保存
        data_set.setdefault(Lx, {}).setdefault("px", []).append(px)
    ...
    for Lx in data_set:
        np.savez_compressed(f"results_pg_{pg}_Lx_{Lx}_Nd_{Nd}_NT_{NT}.npz",
                            px=data_set[Lx]["px"],
    ...
    # 必要ならグラフ描画もここで可能
    time_end = time.time()
    print("pg=",pg,"Np=",Np,"Nd=",Nd,"NT=",NT,"time=",int(time_end-time_start))
