...
for (ip, ids) in my_tasks:
    ...
    for it in range(NT):
        ...
        if it == 0 or it == NT - 1:
            ...
            print(f"Lx=Ly={Lx:>2} rank={rank:>2} ids={ids:>4} px={px:>6.3f} it={it:>4} TEE={TEE}", flush=True)

        key = (round(px, 3),it,ids)
        if key not in local_results:
            local_results[key] = []
        local_results[key].append([px, it, TEE, R2x, R2z, R2x_loop, R2z_loop, dep_x, dep_z])
...
if rank == 0:
    ...
    px_values = sorted(set([key[0] for key in merged_results.keys()]))
    px_to_idx = {px: i for i, px in enumerate(px_values)}
    ...
    for px in px_values:
        ...
        for (px_val, it, ids), vals_list in merged_results.items():
            if px_val != px:
                continue
