...
for (ip, ids) in my_tasks:
    ...
    for it in range(NT):
        ...
        if it % NT0 == 0:
            ...
            print(f"rank={rank:>2} Lx=Ly={Lx:>2} it={it:>4} pg={pg:>6.3f} ids={ids:>4} TEE={TEE:>3} R2z_loop={R2z_loop:>6.3f}", flush=True)
...
time_end = time.time()
print("Np=",Np,"Nd=",Nd,"NT=",NT,"total time=",int(time_end-time_start))
