...
for tmp_i in range(rank, len(L_list), size):
    Np=11
    Lx=L_list[tmp_i]
    Ly=L_list[tmp_i]
    ...
    for ip in range(0,Np,1):
        ...
        for ids in range(Nd):
            ...
            for it in range(NT):
                ...
                if len(TEE_list) > 1:
                    ...
                    TEE_errord=np.std(TEE_list,axis=0, ddof=1)/np.sqrt(len(TEE_list))
                elif len(TEE_list) == 1:
                    TEE_aved = TEE_list[0]
                    TEE_vard = 0.0
                    TEE_errord = 0.0
        ...
        print("Lx=",Lx,"pg=",pg,"TEE_aved=",TEE_aved,"TEE_error=",TEE_errord,"TEE_vard=",TEE_vard,flush=True)

        #with open("Lx="+str(Lx)+"_Ly="+str(Ly)+"_L="+str(L)+"_T="+str(NT)+".lst", 'a') as f:
        #    f.writelines(str(pg)+'\t'+str(TEE_aved)+'\t'+str(TEE_errord)+'\t'+str(TEE_vard)+"\n")
    time_end = time.time()
    print("L=",Lx,"time=",int(time_end-time_start),flush=True)
    ...
    plt.savefig("TEE_graph.png")

"""
# -------------------------------
# gather all data to rank 0
# -------------------------------
all_TEE_ave = comm.gather(TEE_ave, root=0)
all_TEE_error = comm.gather(TEE_error, root=0)
all_TEE_var = comm.gather(TEE_var, root=0)
all_p_list = comm.gather(p_list, root=0)
all_Lx_list = comm.gather(Lx, root=0)  # Lx も渡す（1つのスカラーでも）

# -------------------------------
# rank 0 でのみ plot and save
# -------------------------------
if rank == 0:
    fig, ax = plt.subplots(1,2,figsize=(12,4))
    for i in range(len(all_TEE_ave)):
        ax[0].errorbar(all_p_list[i], all_TEE_ave[i], yerr=all_TEE_error[i], marker='x', capthick=1, capsize=8, lw=1, label=f"Lx=Ly={all_Lx_list[i]}")
        ax[1].plot(all_p_list[i], all_TEE_var[i], 'x', linestyle='dashed', label=f"Lx=Ly={all_Lx_list[i]}")

    ax[0].set_xlabel(r'$p_g$',fontsize=16)
    ax[0].set_ylabel(r'TEE',fontsize=16)
    ax[0].legend(loc=0,fontsize=14)

    ax[1].set_xlabel(r'$p_g$',fontsize=16)
    ax[1].set_ylabel(r'$var[TEE]$',fontsize=16)
    ...
    # npz保存もここ
    np.savez_compressed("TEE_all_data.npz", data_set=all_TEE_ave)
    plt.savefig("TEE_graph.png")
#plt.show()"""
