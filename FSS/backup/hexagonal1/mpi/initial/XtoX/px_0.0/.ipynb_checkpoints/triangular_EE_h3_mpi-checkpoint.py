...
for (tmp_i, ip, ids) in my_tasks:
    ...
    if ip <= (Np - 1) // 2:
        pg = ps + (1 - px - ps) * (2 * ip) / (Np - 1)
    else:
        pg = ps + (1 - px - ps) * (2 * (Np - 1 - ip)) / (Np - 1)
    ...
    for it in range(NT):
        ...
        if it==NT-1:
            ...
            # trajectryの最後のMRの保存用
            if ids==Nd-1:
                np.save(f"MR_Lx_{Lx}_ip_{ip}.npy", MR)
...
# --- 4. MPI集約 ---
    ...
    if rank == 0:
        # zipして1つのリストにする
        results = list(zip(pg_list_all, ave_list_all, err_list_all, var_list_all))
        ...
        if len(results) % 2 == 0:
            # 偶数の場合：前半→後半[::-1]
            results_ordered = results[:half] + results[half:][::-1]
        else:
            # 奇数の場合：中央要素を含めて
            results_ordered = results[:half+1] + results[half+1:][::-1]
    
        # 出力
        for pg, ave, err, var in results_ordered:
